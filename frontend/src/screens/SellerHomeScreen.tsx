...
type MiniGamePhase = "idle" | "running" | "gameover";

function SellerMiniGameCard({ t }: { t: ReturnType<typeof useI18n>["t"] }) {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      setPlayerY((currentY) => {
        ...
        if (jumpQueuedRef.current && currentY <= 1) {
          nextVelocity = 12;
          jumpQueuedRef.current = false;
        }
      ...
      setObstacleX((currentX) => {
        ...
        if (nextX <= -12) {
          ...
          speedRef.current = Math.min(6.2, speedRef.current + 0.12);
          return 100 + Math.random() * 24;
        }
  ...
  useEffect(() => {
    ...
    const obstacleRight = obstacleX + 8;
    const isLowEnough = playerY < 14;

    if (isLowEnough && obstacleRight >= playerLeft && obstacleLeft <= playerRight) {
      setPhase("gameover");
      setBestScore((prev) => Math.max(prev, scoreRef.current));
    }
  }, [obstacleX, phase, playerY]);

  const helperLabel =
    phase === "idle"
      ? t("minigame.tapToPlay")
      : phase === "gameover"
        ? t("minigame.tapToRetry")
        : t("minigame.jumpHint");
    ...
    >
      <VStack align="stretch" spacing={2}>
        <HStack justify="space-between" align="center">
          ...
          <Text fontSize="sm" fontWeight="900" color="surface.800" fontFamily="'SF Mono', monospace">
            {t("minigame.score")} {score}
            {bestScore > 0 ? ` · ${t("minigame.best")} ${bestScore}` : ""}
          </Text>
        </HStack>
        ...
        <Box
          ...
          h="56px"
          borderRadius="16px"
          bg="linear-gradient(180deg, rgba(255,255,255,0.96) 0%, rgba(244,243,239,0.96) 100%)"
          border="1px solid"
        ...
        >
          ...
          <Box
            position="absolute"
            left="18px"
            bottom={`calc(16px + ${playerY}px)`}
            w="14px"
            h="14px"
            bg="rgba(18,18,18,1)"
            sx={{
              clipPath: "polygon(0 100%, 0 18%, 28% 18%, 28% 0, 85% 0, 85% 18%, 100% 18%, 100% 100%, 70% 100%, 70% 54%, 50% 54%, 50% 100%, 22% 100%, 22% 60%, 0 60%)",
          ...
          <Box
            ...
            sx={{
              clipPath: "polygon(0 100%, 0 20%, 16% 20%, 16% 0, 32% 0, 32% 20%, 68% 20%, 68% 0, 84% 0, 84% 20%, 100% 20%, 100% 100%)",
              imageRendering: "pixelated",
            }}
          />
          ...
          {phase !== "running" ? (
            ...
            >
              {phase === "gameover" ? t("minigame.gameOver") : t("minigame.tapToPlay")}
            </Text>
          ) : null}
        </Box>
      </VStack>
    ...
    >
      <Container maxW="container.sm" px={0}>
        <VStack spacing={5} align="stretch">
          ...
          <Box key={sellerMotionKey} className="soft-screen-transition">
            <VStack spacing={5} align="stretch">
              {renderActiveTab()}
              <SellerMiniGameCard t={t} />
            </VStack>
          </Box>
        </VStack>
      </Container>
